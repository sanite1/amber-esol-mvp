...
export type PayoutSort = "newest" | "oldest" | "amount_high" | "amount_low";

interface Props {
  search: string;
  onSearchChange: (val: string) => void;
  statusFilter: PayoutStatusFilter;
  onStatusChange: (val: PayoutStatusFilter) => void;
  sort: PayoutSort;
  onSortChange: (val: PayoutSort) => void;
  totalCount: number;
}

const statusTabs: { value: PayoutStatusFilter; label: string }[] = [
  { value: "all", label: "All" },
...
export default function PayoutsFilter({
  search,
  onSearchChange,
  statusFilter,
  onStatusChange,
    ...
    <div className="bg-white rounded-2xl border border-[#0B2343]/[0.06] p-3 sm:p-4 space-y-3">
      <div className="flex flex-col sm:flex-row gap-2.5">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search by tutor name or payout ID…"
            value={search}
        ...
        <select
          value={sort}
          onChange={(e) => onSortChange(e.target.value as PayoutSort)}

...
import type { RecentSignup } from "../../../lib/types/adminDashboard";
        ...
        {signups.length === 0 ? (
          <div className="py-8 text-center">
            <div className="w-10 h-10 rounded-full bg-[#0B2343]/[0.03] flex items-center justify-center mx-auto mb-2.5">
              <UserPlus size={18} className="text-[#0B2343]/15" />
            </div>
            <p className="text-xs text-[#0B2343]/25">No recent signups</p>
          </div>
        ) : (
          <div className="space-y-1.5">
            {signups.slice(0, 4).map((su) => (
              <Link
                key={su.id}
                to={
                  su.type === "tutor"

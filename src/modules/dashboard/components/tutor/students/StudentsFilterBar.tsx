...
interface Props {
  ...
  onStatusChange: (v: StudentStatusFilter) => void;
  sort: StudentSortOption;
  onSortChange: (v: StudentSortOption) => void;
  count: number;
}

const statusTabs: { value: StudentStatusFilter; label: string }[] = [
  { value: "all", label: "All" },
...
const sortOptions: { value: StudentSortOption; label: string }[] = [
  { value: "recent", label: "Recent Lesson" },
        ...
        <div className="flex gap-1 overflow-x-auto scrollbar-hide">
          {statusTabs.map((tab) => (
            <button
              ...
              className={`shrink-0 px-2.5 sm:px-3 py-1.5 rounded-lg text-[11px] sm:text-xs font-medium transition-colors ${
